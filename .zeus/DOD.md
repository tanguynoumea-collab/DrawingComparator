...
- [ ] Toutes les tâches du cycle (.zeus/tasks.md) sont `done`
- [ ] `dotnet build -c Release` propre sur la cible net8.0-windows (App) + net8.0 (Core, Tests)
- [ ] Tests unitaires : 100 % verts (DrawingComparator.Tests — AlignmentMath, machine à états de calage, compositeur)
- [ ] Zéro warning de sévérité Warning+ au build Release
- [ ] Aucune valeur magique UI (couleurs/tailles hors Themes/Tokens.xaml)
- [ ] Aucun blocage du thread UI (rasterisation et export asynchrones, verrou PDFium global)
- [ ] Les chaînes visibles respectent le vocabulaire du DESIGN_PLAN (Caler les plans, plan de BASE / plan RÉVISÉ, Exporter PNG)

## Spécifique projet

- [ ] Exe publiable : `dotnet publish -r win-x64 --self-contained` sans trimming, pdfium natif embarqué
- [ ] PDF corrompu / protégé / page inexistante → dialogue d'erreur explicite, jamais de crash
- [ ] Matrice d'alignement stockée en 3×2 générale, exprimée en points PDF
